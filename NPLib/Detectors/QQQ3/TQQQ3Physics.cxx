 ...
 *---------------------------------------------------------------------------*
 * Decription:                                                               *
 *  This class hold QQQ3 treated data                                       *
 *                                                                           *
 *---------------------------------------------------------------------------*
 * Comment:                                                                  *
 *                                                                           *
...
ClassImp(TQQQ3Physics)
    ///////////////////////////////////////////////////////////////////////////
    TQQQ3Physics::TQQQ3Physics() {
  m_Rand = new TRandom3();
  EventMultiplicity = 0;
  m_EventData = new TQQQ3Data;
  m_PreTreatedData = new TQQQ3Data;
...
///////////////////////////////////////////////////////////////////////////
void TQQQ3Physics::BuildSimplePhysicalEvent() { BuildPhysicalEvent(); }

///////////////////////////////////////////////////////////////////////////
void TQQQ3Physics::BuildPhysicalEvent() {
  PreTreat();
  ...
  */
  ...
  for (unsigned int i = 0; i < size; ++i) {
    ...
    name = "QQQ3/D" + NPL::itoa(N) + "_STRIP_FRONT" + NPL::itoa(Front) + "_BACK" + NPL::itoa(Back) + "_E";
    ...
    if (Pixel_E != StripFront_OriginalE[couple[i].X()]) {
      ...
      name = "QQQ3/D" + NPL::itoa(N) + "_STRIP_FRONT" + NPL::itoa(Front) + "_BACK" + NPL::itoa(Back) + "_DEADLAYER";
    ...
    // Fall Back option, take the Strip Calibration
      ...
      name = "QQQ3/D" + NPL::itoa(N) + "_STRIP_FRONT" + NPL::itoa(Front) + "_DEADLAYER";
      DeadLayer.push_back(Cal->GetValue(name, 0));
    }
    else {
      Strip_E.push_back(Back_E);
      name = "QQQ3/D" + NPL::itoa(N) + "_STRIP_BACK" + NPL::itoa(Back) + "_DEADLAYER";
      DeadLayer.push_back(Cal->GetValue(name, 0));
    }
...
///////////////////////////////////////////////////////////////////////////
void TQQQ3Physics::PreTreat() {
  ClearPreTreatedData();
  //   Front
  unsigned int sizeFront = m_EventData->GetMultiplicityFront();
  for (unsigned int i = 0; i < sizeFront; i++) {
    if (m_EventData->GetFront_Energy(i) > m_StripFront_E_RAW_Threshold &&
        IsValidChannel("Front", m_EventData->GetFront_DetectorNbr(i), m_EventData->GetFront_StripNbr(i))) {
      double Front_E = fStrip_Front_E(m_EventData, i);
      if (Front_E > m_StripFront_E_Threshold) {
        m_PreTreatedData->SetFront(m_EventData->GetFront_DetectorNbr(i), m_EventData->GetFront_StripNbr(i), Front_E,
                                   m_EventData->GetFront_TimeCFD(i), m_EventData->GetFront_TimeLED(i));
  ...
  for (unsigned int i = 0; i < sizeBack; i++) {
    if (m_EventData->GetBack_Energy(i) > m_StripBack_E_RAW_Threshold &&
        IsValidChannel("Back", m_EventData->GetBack_DetectorNbr(i), m_EventData->GetBack_StripNbr(i))) {
      double Back_E = fStrip_Back_E(m_EventData, i);
      if (Back_E > m_StripBack_E_Threshold) {
        m_PreTreatedData->SetBack(m_EventData->GetBack_DetectorNbr(i), m_EventData->GetBack_StripNbr(i), Back_E,
                                  m_EventData->GetBack_TimeCFD(i), m_EventData->GetBack_TimeLED(i));
  ...
  for (unsigned int i = 0; i < sizePAD; i++) {
    if (m_EventData->GetPAD_Energy(i) > m_PAD_E_RAW_Threshold &&
      ...
      if (PAD_E > m_PAD_E_Threshold) {
        m_PreTreatedData->SetPAD(m_EventData->GetPAD_DetectorNbr(i), PAD_E, m_EventData->GetPAD_TimeCFD(i),
...
///////////////////////////////////////////////////////////////////////////
int TQQQ3Physics ::CheckEvent() {
  return 1; // Regular Event
}

///////////////////////////////////////////////////////////////////////////
vector<TVector2> TQQQ3Physics ::Match_Front_Back() {
  vector<TVector2> ArrayOfGoodCouple;
  // Prevent code from treating very high multiplicity Event
  // Those event are not physical anyway and that improve speed.
  if (m_PreTreatedData->GetMultiplicityFront() > m_MaximumStripMultiplicityAllowed ||
      m_PreTreatedData->GetMultiplicityBack() > m_MaximumStripMultiplicityAllowed)
  ...
  for (unsigned int i = 0; i < mysizeF; i++) {
    for (unsigned int j = 0; j < mysizeB; j++) {
      // cout << "Det Number F/B = " << m_PreTreatedData->GetFront_DetectorNbr(i) << " =? " <<
      // m_PreTreatedData->GetBack_DetectorNbr(j) << endl;
      ...
      if (m_PreTreatedData->GetFront_DetectorNbr(i) == m_PreTreatedData->GetBack_DetectorNbr(j)) {
        ...
        // cout << "abs(F-B)= " << abs( (m_PreTreatedData->GetFront_Energy(i)-m_PreTreatedData->GetBack_Energy(j))/2. )
        // << endl; cout << "Allowed difference = " << m_StripEnergyMatchingNumberOfSigma*m_StripEnergyMatchingSigma <<
        // endl;

        //   Look if energy match
        if (abs((m_PreTreatedData->GetFront_Energy(i) - m_PreTreatedData->GetBack_Energy(j)) / 2.) <
  ...
  // cout << " Good couples = " << ArrayOfGoodCouple.size()  << "  must.be.less.then  " <<
  // m_PreTreatedData->GetMultiplicityFront() << endl;
...
////////////////////////////////////////////////////////////////////////////
bool TQQQ3Physics ::IsValidChannel(const string& DetectorType, const int& telescope, const int& channel) {
...
///////////////////////////////////////////////////////////////////////////
void TQQQ3Physics::ReadAnalysisConfig() {
  bool ReadingStatus = false;

  // path to file
  string FileName = "./configs/ConfigQQQ3.dat";
  ...
  if (!AnalysisConfigFile.is_open()) {
    cout << " No ConfigQQQ3.dat found: Default parameter loaded for Analayis " << FileName << endl;
    return;
  }
  cout << " Loading user parameter for Analysis from ConfigQQQ3.dat " << endl;

  // Save it in a TAsciiFile
  TAsciiFile* asciiConfig = RootOutput::getInstance()->GetAsciiFileAnalysisConfig();
  asciiConfig->AppendLine("%%% ConfigQQQ3.dat %%%");
  ...
  while (!AnalysisConfigFile.eof()) {
    ...
    // search for "header"
    if (LineBuffer.compare(0, 11, "ConfigQQQ3") == 0)
      ReadingStatus = true;
      ...
      if (whatToDo.compare(0, 1, "%") == 0) {
        ...
        if (DataBuffer.find("STRF") != string::npos) {
          ...
          *(m_PADChannelStatus[Detector - 1].begin() + channel - 1) = false;
          ...
          cout << "Warning: detector type for QQQ3 unknown!" << endl;
      }
...
///////////////////////////////////////////////////////////////////////////
void TQQQ3Physics::ReadConfiguration(NPL::InputParser parser) {
...
///////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
void TQQQ3Physics::AddParameterToCalibrationManager() {
  ...
  for (int i = 0; i < m_NumberOfDetector; ++i) {
    ...
    for (int j = 0; j < 24; ++j) {
      ...
      Cal->AddParameter("QQQ3", "D" + NPL::itoa(i + 1) + "_STRIP_FRONT" + NPL::itoa(j + 1) + "_E",
                        "QQQ3_D" + NPL::itoa(i + 1) + "_STRIP_FRONT" + NPL::itoa(j + 1) + "_E");
      Cal->AddParameter("QQQ3", "D" + NPL::itoa(i + 1) + "_STRIP_FRONT" + NPL::itoa(j + 1) + "_DEADLAYER",
                        "QQQ3_D" + NPL::itoa(i + 1) + "_STRIP_FRONT" + NPL::itoa(j + 1) + "_DEADLAYER");
      Cal->AddParameter("QQQ3", "D" + NPL::itoa(i + 1) + "_STRIP_FRONT" + NPL::itoa(j + 1) + "_T",
                        "QQQ3_D" + NPL::itoa(i + 1) + "_STRIP_FRONT" + NPL::itoa(j + 1) + "_T");

      // Pixel Calibration
      for (int k = 0; k < 48; ++k) {
        Cal->AddParameter(
            "QQQ3", "D" + NPL::itoa(i + 1) + "_STRIP_FRONT" + NPL::itoa(j + 1) + "_BACK" + NPL::itoa(k + 1) + "_E",
            "QQQ3_D" + NPL::itoa(i + 1) + "_STRIP_FRONT" + NPL::itoa(j + 1) + "_BACK" + NPL::itoa(k + 1) + "_E");
        Cal->AddParameter("QQQ3",
                          "D" + NPL::itoa(i + 1) + "_STRIP_FRONT" + NPL::itoa(j + 1) + "_BACK" + NPL::itoa(k + 1) +
                              "_DEADLAYER",
                          "QQQ3_D" + NPL::itoa(i + 1) + "_STRIP_FRONT" + NPL::itoa(j + 1) + "_BACK" + NPL::itoa(k + 1) +
                              "_DEADLAYER");
      }
    }
    ...
    for (int j = 0; j < 48; ++j) {
      ...
      Cal->AddParameter("QQQ3", "D" + NPL::itoa(i + 1) + "_STRIP_BACK" + NPL::itoa(j + 1) + "_E",
                        "QQQ3_D" + NPL::itoa(i + 1) + "_STRIP_BACK" + NPL::itoa(j + 1) + "_E");
      Cal->AddParameter("QQQ3", "D" + NPL::itoa(i + 1) + "_STRIP_BACK" + NPL::itoa(j + 1) + "_DEADLAYER",
                        "QQQ3_D" + NPL::itoa(i + 1) + "_STRIP_BACK" + NPL::itoa(j + 1) + "_DEADLAYER");

      Cal->AddParameter("QQQ3", "D" + NPL::itoa(i + 1) + "_STRIP_BACK" + NPL::itoa(j + 1) + "_T",
                        "QQQ3_D" + NPL::itoa(i + 1) + "_STRIP_BACK" + NPL::itoa(j + 1) + "_T");
    }

    for (int j = 0; j < 1; ++j) {
      // Pad Calibration
      Cal->AddParameter("QQQ3", "D" + NPL::itoa(i + 1) + "_PAD_E", "QQQ3_D" + NPL::itoa(i + 1) + "_PAD_E");
      Cal->AddParameter("QQQ3", "D" + NPL::itoa(i + 1) + "_PAD_T", "QQQ3_D" + NPL::itoa(i + 1) + "_PAD_T");
    }
  }
...
///////////////////////////////////////////////////////////////////////////
void TQQQ3Physics::InitializeRootInputRaw() {
  TChain* inputChain = RootInput::getInstance()->GetChain();
  inputChain->SetBranchStatus("QQQ3", true);
  ...
  if (inputChain->FindBranch("fQQQ3_*"))
    inputChain->SetBranchStatus("fQQQ3_*", true);
  inputChain->SetBranchAddress("QQQ3", &m_EventData);
}

///////////////////////////////////////////////////////////////////////////
void TQQQ3Physics::InitializeRootInputPhysics() {
  TChain* inputChain = RootInput::getInstance()->GetChain();
  ...
  inputChain->SetBranchAddress("QQQ3", &m_EventPhysics);
}

///////////////////////////////////////////////////////////////////////////
void TQQQ3Physics::InitializeRootOutput() {
  TTree* outputTree = RootOutput::getInstance()->GetTree();
  outputTree->Branch("QQQ3", "TQQQ3Physics", &m_EventPhysics);
}

////////////////////////////////////////////////////////////////////////////////
/////   Specific to QQQ3Array   ////
void TQQQ3Physics::AddBoxDetector(double Z) {
  // BOX //
  ...
  // Compute
  double BOX_LeftOver1 =
      BOX_PCB_Length - BOX_PCB_Border_ShortSide - BOX_Wafer_Length - BOX_PCB_Slot_Border1 - BOX_PCB_Slot_Width1;
  ...
  double BOX_LeftOver2 =
      BOX_PCB_Length - BOX_PCB_Border_ShortSide - BOX_Wafer_Length - BOX_PCB_Slot_Border2 - BOX_PCB_Slot_Width2;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (Z < 0) { // Up Stream
      if (i == 0) {
        U = TVector3(1, 0, 0);
        V = TVector3(0, 0, 1);
        Strip_1_1 = TVector3(-A1, B1, Z1);
        m_DetectorNormal.push_back(TVector3(0, -1, 0));
      }
      else if (i == 1) {
        U = TVector3(0, 1, 0);
        V = TVector3(0, 0, 1);
        Strip_1_1 = TVector3(-B1, -A1, Z1);
        m_DetectorNormal.push_back(TVector3(1, 0, 0));
      }
      else if (i == 2) {
        U = TVector3(-1, 0, 0);
        V = TVector3(0, 0, 1);
      ...
      if (i == 0) {
        U = TVector3(-1, 0, 0);
        V = TVector3(0, 0, -1);
        Strip_1_1 = TVector3(A2, B2, Z2);
        m_DetectorNormal.push_back(TVector3(0, -1, 0));
      }
      else if (i == 1) {
        U = TVector3(0, -1, 0);
        V = TVector3(0, 0, -1);
        Strip_1_1 = TVector3(-B2, A2, Z2);
        m_DetectorNormal.push_back(TVector3(1, 0, 0));
      }
      else if (i == 2) {
        U = TVector3(1, 0, 0);
...
////////////////////////////////////////////////////////////////////////////////
void TQQQ3Physics::AddQQQDetector(double R, double Phi, double Z) {
...
////////////////////////////////////////////////////////////////////////////////
TVector3 TQQQ3Physics::GetDetectorNormal(const int& i) const { return (m_DetectorNormal[DetectorNumber[i] - 1]); }
////////////////////////////////////////////////////////////////////////////////
TVector3 TQQQ3Physics::GetPositionOfInteraction(const int& i, bool random) const {
...
////////////////////////////////////////////////////////////////////////////////
double TQQQ3Physics::GetDeadLayer(const int& i) const { return DeadLayer[i]; }
////////////////////////////////////////////////////////////////////////////////
void TQQQ3Physics::InitializeStandardParameter() {
  //   Enable all channel
  vector<bool> ChannelStatus;
...
///////////////////////////////////////////////////////////////////////////
namespace QQQ3_LOCAL {
  //   DSSD
  //   Front
  double fStrip_Front_E(const TQQQ3Data* m_EventData, const int& i) {
    static CalibrationManager* Cal = CalibrationManager::getInstance();
    static string name;
    name = "QQQ3/D" + NPL::itoa(m_EventData->GetFront_DetectorNbr(i)) + "_STRIP_FRONT" +
           NPL::itoa(m_EventData->GetFront_StripNbr(i)) + "_E";
    return Cal->ApplyCalibration(name, m_EventData->GetFront_Energy(i));
  }

  double fStrip_Front_T(const TQQQ3Data* m_EventData, const int& i) {
    static CalibrationManager* Cal = CalibrationManager::getInstance();
    static string name;
    name = "QQQ3/D" + NPL::itoa(m_EventData->GetFront_DetectorNbr(i)) + "_STRIP_FRONT" +
           NPL::itoa(m_EventData->GetFront_StripNbr(i)) + "_T";

    return Cal->ApplyCalibration(name, m_EventData->GetFront_TimeCFD(i));
  }

  //   Back
  double fStrip_Back_E(const TQQQ3Data* m_EventData, const int& i) {
    static CalibrationManager* Cal = CalibrationManager::getInstance();
    static string name;
    name = "QQQ3/D" + NPL::itoa(m_EventData->GetBack_DetectorNbr(i)) + "_STRIP_BACK" +
           NPL::itoa(m_EventData->GetBack_StripNbr(i)) + "_E";

    return Cal->ApplyCalibration(name, m_EventData->GetBack_Energy(i));
  }

  double fStrip_Back_T(const TQQQ3Data* m_EventData, const int& i) {
    static CalibrationManager* Cal = CalibrationManager::getInstance();
    static string name;
    name = "QQQ3/D" + NPL::itoa(m_EventData->GetBack_DetectorNbr(i)) + "_STRIP_BACK" +
           NPL::itoa(m_EventData->GetBack_StripNbr(i)) + "_T";

    return Cal->ApplyCalibration(name, m_EventData->GetFront_TimeCFD(i));
  }

  //   PAD
  double fPAD_E(const TQQQ3Data* m_EventData, const int& i) {
    static CalibrationManager* Cal = CalibrationManager::getInstance();
    static string name;
    name = "QQQ3/D" + NPL::itoa(m_EventData->GetPAD_DetectorNbr(i)) + "_PAD_E";

    return Cal->ApplyCalibration(name, m_EventData->GetPAD_Energy(i));
  }

  double fPAD_T(const TQQQ3Data* m_EventData, const int& i) {
    static CalibrationManager* Cal = CalibrationManager::getInstance();
    static string name;
    name = "QQQ3/D" + NPL::itoa(m_EventData->GetPAD_DetectorNbr(i)) + "_PAD_T";
...
////////////////////////////////////////////////////////////////////////////////
NPL::VDetector* TQQQ3Physics::Construct() { return (NPL::VDetector*)new TQQQ3Physics(); }
...
class proxy_sharc {
 public:
  proxy_sharc() {
    NPL::DetectorFactory::getInstance()->AddToken("QQQ3", "QQQ3");
    NPL::DetectorFactory::getInstance()->AddDetector("QQQ3", TQQQ3Physics::Construct);
  }
};
