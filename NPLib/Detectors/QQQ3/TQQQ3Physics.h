...
class TQQQ3Physics : public TObject, public NPL::VDetector {
 public:
  TQQQ3Physics();
  ~TQQQ3Physics(){};
 ...
 public: //   Specific to QQQ3 Array
  ...
  // Give and external TMustData object to TQQQ3Physics. Needed for online analysis for example.
  void SetRawDataPointer(TQQQ3Data* rawDataPointer) { m_EventData = rawDataPointer; }
  // Retrieve raw and pre-treated data
  TQQQ3Data* GetRawData() const { return m_EventData; }
  TQQQ3Data* GetPreTreatedData() const { return m_PreTreatedData; }

  // Use to access the strip position
  inline double GetStripPositionX(const int& N, const int& Front, const int& Back) const {
    return m_StripPositionX[N - 1][Front - 1][Back - 1];
  };
  inline double GetStripPositionY(const int& N, const int& Front, const int& Back) const {
    return m_StripPositionY[N - 1][Front - 1][Back - 1];
  };
  inline double GetStripPositionZ(const int& N, const int& Front, const int& Back) const {
    return m_StripPositionZ[N - 1][Front - 1][Back - 1];
  };
 ...
 private:                       //   Root Input and Output tree classes
  TQQQ3Data* m_EventData;       //!
  TQQQ3Data* m_PreTreatedData;  //!
  TQQQ3Physics* m_EventPhysics; //!
 ...
 public: // Static constructor to be passed to the Detector Factory
  ...
  ClassDef(TQQQ3Physics, 1) // QQQ3Physics structure
};

namespace QQQ3_LOCAL {
  //   DSSD
  //   Front
  double fStrip_Front_E(const TQQQ3Data* Data, const int& i);
  double fStrip_Front_T(const TQQQ3Data* Data, const int& i);

  //   Back
  double fStrip_Back_E(const TQQQ3Data* Data, const int& i);
  double fStrip_Back_T(const TQQQ3Data* Data, const int& i);

  //   PAD
  double fPAD_E(const TQQQ3Data* Data, const int& i);
  double fPAD_T(const TQQQ3Data* Data, const int& i);
